...
import { PaymentMethodListQuery } from '../PaymentMethodListQuery.graphql';
	...
	for (let index = 0; index < items; index++) {
		...
		edges.push({
			...
			node: {
				...
				instructions: faker.lorem.sentence(faker.random.number({ min: 2, max: 10 })),
...
export const PaymentMethodListQueryMocks = {
	...
	success: (avatarSize = 142, itemsPerPage = 10) => [
		...PaymentMethodListQueryMocks.successInfinite(avatarSize, itemsPerPage),
		...PaymentMethodListQueryMocks.successPagination(avatarSize, itemsPerPage),
	],
